...
Employee Brain — AI-generated skill assessment quiz.
10-15 MCQ questions based on domain + experience.
Detects if skills are genuine. Identifies gaps. Shows improvement videos.
"""
import sys, os
...
def render():
    ...
    if "quiz_state" not in st.session_state:
        st.session_state.quiz_state = "setup"  # setup | quiz | results
    ...
    if st.session_state.quiz_state == "setup":
        ...
        with col2:
            skills = st.text_area("Your claimed skills",
                placeholder="Python, TensorFlow, LangChain, Docker, AWS",
                height=100)
            target = st.text_input("Target role / job you're applying for",
    ...
    # ── QUIZ SCREEN ────────────────────────────────────────────────────────────
        ...
        diff_color = {"easy":"🟢","medium":"🟡","hard":"🔴"}.get(q.get("difficulty","medium"),"🟡")
        st.markdown(f"{diff_color} *{q.get('difficulty','medium').capitalize()} — Topic: {q.get('topic','')}*")
        ...
        with col2:
            if current_q < total - 1:
                ...
                if st.button("✅ Submit Quiz", type="primary", use_container_width=True):
                    ...
                    if answered < total:
                        st.warning(f"You answered {answered}/{total}. Answer all questions before submitting.")
                    else:
                        st.session_state.quiz_state = "results"
                        st.rerun()

        with col3:
            answered = len(answers) + (1 if selected and qn_key not in answers else 0)
        ...
        for i, ques in enumerate(questions):
            ...
            with cols[i]:
                ...
                if st.button(status, key=f"nav_{i}", use_container_width=True):
                    if selected:
                        st.session_state.quiz_answers[qn_key] = selected[0]
    ...
    # ── RESULTS SCREEN ─────────────────────────────────────────────────────────
        ...
        r_color = {"Ready":"#2d9e2d","Almost Ready":"#e07000","Needs More Prep":"#cc2222"}.get(readiness,"#666")

        st.markdown(f"""
        <div style="border:2px solid {color};border-radius:12px;padding:24px;margin-bottom:20px;
                    text-align:center;">
        ...
        tab1, tab2, tab3, tab4 = st.tabs(["📊 Results", "📝 Review Answers", "📺 Fix Weak Areas", "🎯 Study Plan"])
        ...
        with tab2:
            ...
            for q in questions:
                ...
                with st.expander(f"{emoji} Q{q['question_number']}: {q['question'][:70]}..."):
                    ...
                    if not is_correct:
                        st.markdown(f"**Correct answer:** {correct_ans} — {q['options'].get(correct_ans,'')}")
        ...
        with tab3:
            ...
            if weak_areas:
                ...
                st.markdown("*Watch these to fill your knowledge gaps:*")
                for skill in weak_areas[:5]:
                    st.markdown(f"#### 🔴 {skill}")
                    ...
                    for v in vids:
                        ...
                        if vid_id:
                            ...
                            if st.button("💬 Chat about this video", key=f"chat_quiz_{vid_id}"):
                ...
                st.success("🎉 No major weak areas! You're well prepared.")
